document.addEventListener('DOMContentLoaded', () => {
    ...
    let state = {
        ...
        cameraStream: null,
        // --- INICIO MEJORA 2 Y 3 (PWA) ---
        availableCameras: [],
        selectedPhotoCameraId: null, // MEJORA PWA
        selectedQrCameraId: null, // MEJORA PWA
        // --- FIN MEJORA 2 Y 3 ---
        readOnlyMode: false,
        activityLog: [],
    ...
    const elements = {
        ...
        photo: {
            ...
            cameraSelect: document.getElementById('photo-camera-select') // MEJORA 2
        },
        ...
        qrCameraSelect: document.getElementById('qr-camera-select'), // MEJORA 3
        areaClosure: {
    ...
    async function resetInventoryState() {
        ...
        state = {
            ...
            serialNumberCache: new Set(),
            cameraStream: null,
            availableCameras: state.availableCameras, // Mantener las cámaras detectadas
            selectedPhotoCameraId: null, // Reiniciar cámaras
            selectedQrCameraId: null, // Reiniciar cámaras
            readOnlyMode: false,
            activityLog: [],
            institutionalReportCheckboxes: {},
            actionCheckboxes: {
    ...
    function saveState() {
        ...
        try {
            ...
            delete stateToSave.availableCameras; // No es necesario guardar esto
    ...
    async function startQrScanner() {
        ...
        // --- MEJORA PWA: Priorizar cámara guardada o trasera ---
        let cameraIdOrConfig;
        const selectedCameraId = elements.qrCameraSelect.value; // El select ya tiene el valor guardado
        
        if (selectedCameraId) {
            cameraIdOrConfig = selectedCameraId;
        } else {
            // Si no hay ID, intentar con 'environment' (trasera)
        ...
        html5QrCode.start(
            ...
            if (typeof cameraIdOrConfig === 'object') {
                ...
                showToast('Error al iniciar la cámara seleccionada.', 'error');
                console.error("Error al iniciar el escaner QR (por deviceId): ", err);
                stopQrScanner();
            }
        });
        
        // --- MEJORA PWA: Guardar la cámara que realmente se está usando ---
        // Esperar un momento a que el stream se active para obtener el deviceId
        setTimeout(() => {
            if (html5QrCode && html5QrCode.isScanning) {
                try {
                    const currentTrack = html5QrCode.getRunningTrackCapabilities();
                    const currentCameraId = currentTrack?.deviceId;
                    if (currentCameraId && state.availableCameras.some(c => c.deviceId === currentCameraId)) {
                        elements.qrCameraSelect.value = currentCameraId;
                        state.selectedQrCameraId = currentCameraId;
                    }
                } catch (e) {
                    console.warn("No se pudo obtener el track de la cámara QR para guardar el ID.", e);
                }
            }
        }, 500); // 500ms de espera
        // --- FIN MEJORA PWA ---
    ...
    async function getCameraDevices() {
        try {
            ...
            // No mostrar toast aquí, puede ser molesto si solo es un chequeo inicial
        }
    }
    
    function populateCameraSelectors() {
        const { qrCameraSelect, photo: { cameraSelect } } = elements;
        const selectors = [qrCameraSelect, cameraSelect];
        
        selectors.forEach(select => {
            select.innerHTML = '';
            if (state.availableCameras.length === 0) {
                select.innerHTML = '<option value="">No hay cámaras disponibles</option>';
                return;
            }
            state.availableCameras.forEach((camera, index) => {
                let label = camera.label || `Cámara ${index + 1}`;
                if (label.toLowerCase().includes('back')) label += ' (Trasera)';
                if (label.toLowerCase().includes('front')) label += ' (Frontal)';
                if (label.toLowerCase().includes('telephoto')) label += ' (Zoom)';
                
                const option = new Option(label, camera.deviceId);
                select.add(option);
            });

            // --- MEJORA PWA: Recordar selección de cámara ---
            if (select.id === 'qr-camera-select') {
                if (state.selectedQrCameraId && state.availableCameras.some(c => c.deviceId === state.selectedQrCameraId)) {
                    select.value = state.selectedQrCameraId;
                } else {
                    // Si no hay nada guardado, intentar adivinar la trasera/zoom
                    const zoomCamera = state.availableCameras.find(c => c.label.toLowerCase().includes('telephoto') || c.label.toLowerCase().includes('zoom') || c.label.toLowerCase().includes('trasera') || c.label.toLowerCase().includes('back'));
                    select.value = zoomCamera ? zoomCamera.deviceId : (state.availableCameras[0]?.deviceId || '');
                }
            } else if (select.id === 'photo-camera-select') {
                if (state.selectedPhotoCameraId && state.availableCameras.some(c => c.deviceId === state.selectedPhotoCameraId)) {
                    select.value = state.selectedPhotoCameraId;
                } else {
                    // Si no hay nada guardado, intentar adivinar la trasera/ancha
                    const wideCamera = state.availableCameras.find(c => c.label.toLowerCase().includes('wide') || c.label.toLowerCase().includes('back') || c.label.toLowerCase().includes('trasera'));
                    select.value = wideCamera ? wideCamera.deviceId : (state.availableCameras[0]?.deviceId || '');
                }
            }
            // --- FIN MEJORA PWA ---
        });
    ...
    async function startCamera() {
        if (state.readOnlyMode) return;
        const { cameraStream, uploadContainer, cameraViewContainer, cameraSelect } = elements.photo;
        ...
        if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
            let constraints;
            // --- MEJORA PWA: Priorizar cámara guardada o trasera ---
            const selectedCameraId = cameraSelect.value; // El select ya tiene el valor guardado gracias a populateCameraSelectors
            
            if (selectedCameraId) {
                constraints = { video: { deviceId: { exact: selectedCameraId } } };
            } else {
                // Si no hay ID seleccionado, intentar con 'environment' (trasera)
            ...
            try {
                ...
                const currentCameraId = currentTrack.getSettings().deviceId;
                if (currentCameraId && state.availableCameras.some(c => c.deviceId === currentCameraId)) {
                    cameraSelect.value = currentCameraId;
                    state.selectedPhotoCameraId = currentCameraId;
                }
                // --- FIN MEJORA PWA ---
                ...
                if (constraints.video.facingMode) {
                    ...
                    showToast('No se pudo acceder a la cámara seleccionada.', 'error');
                    console.error("Error al acceder a la cámara (por deviceId): ", err);
                }
            }
        } else {
    ...
    function initialize() {
        ...
        getCameraDevices(); // MEJORA: Detectar cámaras al inicio.
        ...
        // --- INICIO MEJORA 1: LÓGICA PARA OCULTAR HEADER ---
        ...
        // MEJORA PWA: Guardar selección y reiniciar
        elements.photo.cameraSelect.addEventListener('change', () => {
            state.selectedPhotoCameraId = elements.photo.cameraSelect.value;
            saveState();
            startCamera();
        });
        
        // MEJORA PWA: Guardar selección y reiniciar
        elements.qrCameraSelect.addEventListener('change', () => {
            state.selectedQrCameraId = elements.qrCameraSelect.value;
            saveState();
            startQrScanner();
        });
